...
class ResNetHed(nn.Module):
    def __init__(self, args):
        ...
        elif '50' in self.hparams.backbone_model:
            base_model = resnet.resnet50(pretrained=self.hparams.pretrained)
        elif '101' in self.hparams.backbone_model:
            base_model = resnet.resnet101(pretrained=self.hparams.pretrained)
        ...
        if '18' in self.hparams.backbone_model or '34' in self.hparams.backbone_model:
        ...
        elif '50' in self.hparams.backbone_model or '101' in self.hparams.backbone_model:
            self.up1_4 = nn.ConvTranspose2d(1024, 512, 3, stride=2, padding=1, output_padding=1)
            self.up1_3 = nn.ConvTranspose2d(512, 256, 3, stride=2, padding=1, output_padding=1)
            self.up1_2 = nn.ConvTranspose2d(256, 64, 3, stride=2, padding=1, output_padding=1)
            self.up1_1 = nn.ConvTranspose2d(64, 64, 3, stride=2, padding=1, output_padding=1)

            self.up2_3 = nn.ConvTranspose2d(512, 256, 3, stride=2, padding=1, output_padding=1)
            self.up2_2 = nn.ConvTranspose2d(256, 64, 3, stride=2, padding=1, output_padding=1)
            self.up2_1 = nn.ConvTranspose2d(64, 64, 3, stride=2, padding=1, output_padding=1)

            self.up3_2 = nn.ConvTranspose2d(256, 64, 3, stride=2, padding=1, output_padding=1)
            self.up3_1 = nn.ConvTranspose2d(64, 64, 3, stride=2, padding=1, output_padding=1)

            self.up4_1 = nn.ConvTranspose2d(64, 64, 3, stride=2, padding=1, output_padding=1)

            self.vote1_4 = convrelu(1024, 512, 3, 1)
            self.vote1_3 = convrelu(512, 256, 3, 1)
            self.vote1_2 = convrelu(128, 64, 3, 1)
            self.vote1_1 = convrelu(128, 64, 3, 1)

            self.vote2_3 = convrelu(512, 256, 3, 1)
            self.vote2_2 = convrelu(128, 64, 3, 1)
            self.vote2_1 = convrelu(128, 64, 3, 1)
    ...
    def forward(self, input):
        ...
        #UP LAYER 2
        up2_3_1 = self.up2_3(up1_4_3)
        up2_3_2 = torch.cat((up1_3_3, up2_3_1), dim=1)
        up2_3_3 = self.vote2_3(up2_3_2)

        up2_2_1 = self.up2_2(up2_3_3)
        up2_2_2 = torch.cat((up1_2_3, up2_2_1), dim=1)
        up2_2_3 = self.vote2_2(up2_2_2)

        up2_1_1 = self.up2_1(up2_2_3)
        up2_1_2 = torch.cat((up1_1_3, up2_1_1), dim=1)
        up2_1_3 = self.vote2_1(up2_1_2)

        res2 = self.score2(up2_1_3)

        #UP LAYER 3
        up3_2_1 = self.up3_2(up2_3_3)
        up3_2_2 = torch.cat((up2_2_3, up3_2_1), dim=1)
        up3_2_3 = self.vote3_2(up3_2_2)

        up3_1_1 = self.up3_1(up3_2_3)
        up3_1_2 = torch.cat((up2_1_3, up3_1_1), dim=1)
        up3_1_3 = self.vote3_1(up3_1_2)

        res3 = self.score2(up3_1_3)

        #UP LAYER 4
        up4_1_1 = self.up4_1(up3_2_3)
        up4_1_2 = torch.cat((up3_1_3, up4_1_1), dim=1)
