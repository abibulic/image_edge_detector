...
class VggHed(nn.Module):
    def __init__(self, args):
        ...
        self.upsample3 = nn.ConvTranspose2d(256, 256, 3, stride=4, padding=1, output_padding=3)
        self.upsample4 = nn.ConvTranspose2d(512, 512, 3, stride=8, padding=1, output_padding=7)
        self.upsample5 = nn.ConvTranspose2d(512, 512, 3, stride=16, padding=1, output_padding=15)

        self.upsample22 = nn.ConvTranspose2d(128, 128, 3, stride=2, padding=1)
        self.upsample33 = nn.ConvTranspose2d(128, 256, 3, stride=2, padding=1)
        self.upsample44 = nn.ConvTranspose2d(256, 512, 3, stride=2, padding=1)
        self.upsample55 = nn.ConvTranspose2d(512, 512, 3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        upsample55 = self.upsample22(conv5_3)
        upsample44 = self.upsample33(upsample55)
        upsample33 = self.upsample44(upsample44)
        upsample22 = self.upsample55(upsample33)

        upsample2 = self.upsample2(conv2_2)
        upsample3 = self.upsample3(conv3_3)
        upsample4 = self.upsample4(conv4_3)
        upsample5 = self.upsample5(conv5_3)
        
        upsample2 += upsample22
        upsample3 += upsample33
        upsample4 += upsample44
        upsample5 += upsample55

        so1 = self.score_dsn1(conv1_2)
        so2 = self.score_dsn2(upsample2)
        so3 = self.score_dsn3(upsample3)
        so4 = self.score_dsn4(upsample4)
        so5 = self.score_dsn5(upsample5)        
